...
j = 3 # plotly counts rows from the bottom up
for i in range(len(extracted_metrics)):
    metric = extracted_metrics.iloc[i,1]
    annotation = extracted_metrics.iloc[i,0]
    fig.add_hline(y=metric, line_dash="dot", row=j,annotation_text="baseline " + annotation, annotation_position="bottom right")
    j=j-1
